  ...
  setupUpdater() {
    ...
    window.electronAPI.onUpdaterEvent((data) => {
      ...
      if (data.event === 'available') {
        updateMsg.innerHTML = `Version <b>${data.data.version}</b> ist verfügbar!<br>Möchtest du sie jetzt herunterladen?`;
        ...
        btnDismiss.style.display = 'inline-block';        btnDismiss.innerHTML = 'Später';
        ...
        updateMsg.innerHTML = `Update erfolgreich heruntergeladen.<br>App jetzt neu starten?`;
        ...
        if (window.isManualUpdateCheck) {
          updateMsg.innerHTML = `Die App ist auf dem neuesten Stand!<br>Du hast bereits die aktuellste Version.`;
          btnAction.innerHTML = 'OK';
          btnAction.disabled = false;
          btnAction.onclick = () => {
            updateUi.classList.add('hidden');
          };
          btnDismiss.style.display = 'none';
          updateUi.classList.remove('hidden');
        }
        
        // Reset settings button if it exists
        const updateBtn = document.getElementById('settings-update-btn');
        if (updateBtn) {
          updateBtn.innerHTML = '<i class="fas fa-check"></i> Auf dem neuesten Stand';
          updateBtn.disabled = true;
        }
      } else if (data.event === 'error') {
        if (window.isManualUpdateCheck) {
          updateMsg.innerHTML = `Fehler bei der Suche nach Updates.<br>Bitte später erneut versuchen.`;
          btnAction.innerHTML = 'OK';
          btnAction.disabled = false;
          btnAction.onclick = () => { updateUi.classList.add('hidden'); };
          btnDismiss.style.display = 'none';
          updateUi.classList.remove('hidden');
        }
        const updateBtn = document.getElementById('settings-update-btn');
